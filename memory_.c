...
fb_t* create_block_at(int index, size_t size) {
	...
	fb_t *after = before == NULL ? get_first_block() : before -> next;
...
void mem_init(size_t size) {
	...
	fb_t *first_block = get_first_block();
	*first_block = (fb_t) {(size_t) 0, NULL};
